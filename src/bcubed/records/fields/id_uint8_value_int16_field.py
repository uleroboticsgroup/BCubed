...
It contains the IdUint8ValueInt16Field class, which inherits from BaseIdNumberValueNumberField
...
from bcubed.utilities.integer_help import is_valid_uint8, is_valid_int16


class IdUint8ValueInt16Field(BaseIdNumberValueNumberField):
    """
    It contains the dictionary that the id-uint8/value-int16 fields can contain and its value
    ...
    def __setitem__(self, key: str, value):
        if key == IdValueFields.FIELD_ID and not is_valid_uint8(value):
            raise ValueError(
                f"IdUint8ValueInt16Field. {key} value is not valid: {value}")

        value = self._decompress_value_to_int(key, value)
        if key is IdValueFields.FIELD_VALUE and not is_valid_int16(value):
            raise ValueError(
                f"IdUint8ValueInt16Field. {key} value is not valid: {value}")
