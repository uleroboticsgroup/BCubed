...
It contains the class GivenAnIdUint8ValueStringField which inherits from TestCase and performs all
the IdUint8ValueStringField tests.
"""
...
class GivenAnIdUint8ValueStringField (TestCase):
    """
    It contains the test suite related with IdUint8ValueStringField class.
    Add tests as required.
    """

    DICTIONARY_LENGTH = 2
    CLASS_ID = "IdUint8ValueStringField. "

    def setUp(self) -> None:
        self.uint8_string_field = IdUint8ValueStringField()

        return super().setUp()

    def tearDown(self) -> None:
        del self.uint8_string_field

        return super().tearDown()

    def test_when_creating_field_then_its_values_are_default_ones(self):
        self.assertEqual(len(self.uint8_string_field), self.DICTIONARY_LENGTH)

        self.assertEqual(
            self.uint8_string_field[IdValueFields.FIELD_ID], DEFAULT_NUMBER_VALUE)
        self.assertEqual(
            self.uint8_string_field[IdValueFields.FIELD_VALUE], DEFAULT_STRING_VALUE)

    def test_when_adding_new_key_then_it_is_not_added_and_an_exception_raises(self):
        with self.assertRaises(KeyError) as context:
            self.uint8_string_field[TEST_STRING] = VALID_NUMBER_VALUE
    ...
    def test_when_updating_id_key_value_with_invalid_value_then_it_is_not_updated_and_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_string_field[IdValueFields.FIELD_ID] = INVALID_NUMBER_VALUE
    ...
    def test_when_updating_id_key_value_with_valid_value_then_it_is_updated(self):
        self.uint8_string_field[IdValueFields.FIELD_ID] = VALID_NUMBER_VALUE
    ...
    def test_when_updating_value_key_value_with_invalid_value_then_it_is_not_updated_and_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_string_field[IdValueFields.FIELD_VALUE] = INVALID_STRING_VALUE
    ...
    def test_when_updating_value_key_value_with_valid_value_then_it_is_updated(self):
        self.uint8_string_field[IdValueFields.FIELD_VALUE] = VALID_STRING_VALUE
