...
It contains the class GivenAnIdUint8ValueUint16Field which inherits from TestCase and performs all
the IdUint8ValueUint16Field tests.
"""
...
class GivenAnIdUint8ValueUint16Field (TestCase):
    """
    It contains the test suite related with IdUint8ValueUint16Field class.
    Add tests as required.
    """

    DICTIONARY_LENGTH = 2
    CLASS_ID = "IdUint8ValueUint16Field. "

    def setUp(self) -> None:
        self.uint8_uint16_field = IdUint8ValueUint16Field()

        return super().setUp()

    def tearDown(self) -> None:
        del self.uint8_uint16_field

        return super().tearDown()

    def test_when_creating_field_then_its_values_are_default_ones(self):
        self.assertEqual(len(self.uint8_uint16_field), self.DICTIONARY_LENGTH)

        self.assertEqual(
            self.uint8_uint16_field[IdValueFields.FIELD_ID], DEFAULT_NUMBER_VALUE)
        self.assertEqual(
            self.uint8_uint16_field[IdValueFields.FIELD_VALUE], DEFAULT_NUMBER_VALUE)

    def test_when_adding_new_key_then_it_is_not_added_and_an_exception_raises(self):
        with self.assertRaises(KeyError) as context:
            self.uint8_uint16_field[TEST_STRING] = TEST_STRING
    ...
    def test_when_updating_id_key_value_with_invalid_value_then_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_uint16_field[IdValueFields.FIELD_ID] = INVALID_NUMBER_VALUE
    ...
    def test_when_updating_id_key_value_with_smaller_value_than_allowed_then_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_uint16_field[IdValueFields.FIELD_ID] = UintRanges.UINT8_MIN - 1
    ...
    def test_when_updating_id_key_value_with_greater_value_than_allowed_then_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_uint16_field[IdValueFields.FIELD_ID] = UintRanges.UINT8_MAX + 1
    ...
    def test_when_updating_id_key_value_with_valid_value_then_it_is_updated(self):
        self.uint8_uint16_field[IdValueFields.FIELD_ID] = VALID_NUMBER_VALUE
    ...
    def test_when_updating_value_key_value_with_invalid_value_then_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_uint16_field[IdValueFields.FIELD_VALUE] = INVALID_NUMBER_VALUE
    ...
    def test_when_updating_value_key_value_with_smaller_value_than_allowed_then_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_uint16_field[IdValueFields.FIELD_VALUE] = UintRanges.UINT16_MIN - 1
        ...
        self.assertEqual(context.exception.args[0], self.CLASS_ID + VALUE_NOT_VALID_ERROR.format(
            IdValueFields.FIELD_VALUE, UintRanges.UINT16_MIN - 1))

    def test_when_updating_value_key_value_with_greater_value_than_allowed_then_an_exception_raises(self):
        with self.assertRaises(ValueError) as context:
            self.uint8_uint16_field[IdValueFields.FIELD_VALUE] = UintRanges.UINT16_MAX + 1
        ...
        self.assertEqual(context.exception.args[0], self.CLASS_ID + VALUE_NOT_VALID_ERROR.format(
            IdValueFields.FIELD_VALUE, UintRanges.UINT16_MAX + 1))

    def test_when_updating_value_key_value_with_valid_value_then_it_is_updated(self):
        self.uint8_uint16_field[IdValueFields.FIELD_VALUE] = VALID_NUMBER_VALUE
